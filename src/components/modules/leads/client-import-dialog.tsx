...
import { bulkImportClientsAction, type ImportRow, type ImportResult } from "@/app/(app)/leads/clients/actions";
...
/**
 * The "sync my existing client roster" path — bulk-add clients that never
 * went through the lead pipeline (e.g. schools already under contract
 * before this app existed), from an uploaded CSV or a raw Sheets paste.
 */
export function ClientImportDialog() {
  ...
  async function handleImport() {
    setImporting(true);
    try {
      const res = await bulkImportClientsAction(rows);
      setResult(res);
      if (res.created > 0) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Sync your existing clients</DialogTitle>
          <DialogDescription>
            Columns: {EXPECTED_COLUMNS.join(", ")}. Only <code>name</code> is required. Status
            defaults to Active if left blank or unrecognized.
          </DialogDescription>
        </DialogHeader>
        ...
        {mode === "file" ? (
          ...
          <div className="space-y-1.5">
            <Textarea
              placeholder={`Select your client list in Google Sheets (including the header row), copy (Cmd/Ctrl+C), then paste here.\n\nname\tcompany\tstatus\n...`}
              value={pasteText}
                ...
                <TableBody>
                  {rows.slice(0, 8).map((r, i) => (
                    <TableRow key={i} className="hover:bg-muted/40">
                      ...
                      <TableCell className="text-muted-foreground">{r.status || "ACTIVE"}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
          ...
          <div className="space-y-1.5 rounded-lg border bg-muted/20 p-3 text-sm">
            ...
            {result.duplicates > 0 && (
              <p className="text-amber-600 dark:text-amber-400">
                {result.duplicates} matched an existing client by name/company — imported anyway, review for duplicates.
              </p>
            )}
